...
#pragma region  >> MAXUSER - LOJINHA
		...
		if (SecCounter % 1800 == 0)
		{
			for (int i = 0; i < MAX_USER; i++)
			{
				...
				SendClientSignalParmLojinha(i, ESCENE_FIELD, _MSG_StartTime, pMob[i].CheckLojinha, pUser[i].TradeMode);
				...
				{
					pUser[i].Donate += 1;
					pMob[i].CheckLojinha = TIMERLOJINHA;//600 = 10 minutos
					SendClientMessage(i, "Voce ganhou +1 ponto de donate digite /donate para ver seu saldo");
					SendEtc(i);
					sprintf(temp, "Seu saldo atual e %d", pUser[i].Donate);
					SendClientMessage(i, temp);
				}
			}
		}
		...
		time(&now);
		when = *localtime(&now);
#pragma region EVENTO PVP ARMIA DROP	
		if (when.tm_hour == 16 && when.tm_min == 00 && when.tm_sec == 00)
		{
			//FECHA O PORTÃO E LIMPA O PVP
			int i = 0;
			...
			ClearArea(2124, 2140, 2147, 2156); //Limpa o pvp de armia
			sprintf(temp, "PORTAO FECHADO, QUE A BATALHA COMEÇE!!!");
			SendNotice(temp);
			return;
		}

		if ((when.tm_hour == 16) && (when.tm_sec == 10 || when.tm_sec == 20 || when.tm_sec == 30 || when.tm_sec == 40 || when.tm_sec == 50))
		{
			//DROPA O ITEM 
			STRUCT_ITEM Item;
			...
			if (_rd < 25)
		{
			Item.sIndex = 4010; //PRIMEIRO ITEM
		}
		else if (_rd < 50){
			Item.sIndex = 4029;} //SEGUNDO ITEM

		else if (_rd < 75){
			Item.sIndex = 4028;} //TERCEIRO ITEM 

		else{
			Item.sIndex = 3318;} //QUARTO ITEM 

			int rd = rand() % 15;
			int randx = 2124 + rd; //COORDENADA X
			int randy = 2146 + rd; //COORDENADA Y
			pItem[empty].Delay = 5; //Tempo que o item fica no chão
			CreateItem(randx, randy, &Item, rand() % 5, 1);

			sprintf(temp, "O item %s foi criado em x: %d y: %d", g_pItemList[Item.sIndex].Name, randx, randy);
			SendNotice(temp);

			return;
		}
		else if (when.tm_hour == 16 && when.tm_min == 10 && when.tm_sec == 59)
		{
			//ABRE O PORTÃO
			...
			sprintf(temp, "O EVENTO DROP COMECOU , ACESSO AO PVP DE ARMIA LIBERADO");
			SendNotice(temp);
			return;
...
#pragma endregion
		...
		//faz com que quando tire a Fada Agrupadora vc seja teleportado para fora
#pragma region Área de Evento com Fada Agrupadora
		if (((pMob[bb].TargetX) / 128) == 10 && ((pMob[bb].TargetY) / 128) == 11)
		{
			if (pMob[bb].MOB.Equip[13].sIndex != 3905)
				DoRecall(bb);
		}
#pragma endregion
...
#pragma endregion
	...
	if(SecCounter % 12 == 0)
	{
		...
		//RvRStat
		SendShortSignalParm2Area(1017, 1911, 1290, 2183, ESCENE_FIELD, _MSG_MobCount, g_pRvrWar.Points[1], g_pRvrWar.Points[0]);//
	}
...
#pragma region >> Reset Quest
	...
	if(SecCounter % 1200 == 0)
	{
		...
		ClearArea(3604, 3604, 3691, 3690);//Lanhouse N  
		ClearArea(3732, 3476, 3819, 3563);//Lanhouse M   
		ClearArea(3604, 3860, 3946, 3690);//Lanhouse A 

		SendSpkMsg(1, "Todas as quest foram resetadas", TNColor::Red, true);
	...
	if(SecCounter % 120 == 0)
	{
		for(int user = 0; user < MAX_USER; user++)
		{
			...
			if (pMob[user].MOB.Equip[14].EsferaPet() && BASE_CheckItemDate(&pMob[user].MOB.Equip[14]))
			{
			...
			if (pMob[user].MOB.Equip[12].EquipCostume() && BASE_CheckItemDate(&pMob[user].MOB.Equip[12]))
			{
	...
	if (SecCounter % 2 == 0)
	{
		...
		if ((when.tm_wday == 6 && when.tm_hour == 22 && when.tm_min == 0) && when.tm_sec >= 0 && when.tm_sec <= 2 && KefraLive)
		{
			KefraLive = 0;

			//Kefra
			GenerateMob(KEFRA_BOSS, 0, 0);
			sprintf(temp, "KEFRA Acabou de Nascer Mate Agora !!! ");
			SendNotice(temp);
		...
		if ((when.tm_hour == 10 && when.tm_min == 5) && when.tm_sec >= 0 && when.tm_sec <= 2)
		{


			//BOSS_ext1
			GenerateMob(BOSS_ext1, 0, 0);
			//CreateMob("DRAGON_NEST", 1080, 1690, "npc", 0);
			sprintf(temp, "TK ROL Acabou de Nascer Mate Agora !!! Altar x: 2484 y: 1763 ");
			SendNotice(temp);


		}

		if ((when.tm_hour == 13 && when.tm_min == 5) && when.tm_sec >= 0 && when.tm_sec <= 2)
		{


			//BOSS_ext2
			GenerateMob(BOSS_ext2, 0, 0);
			sprintf(temp, "FM ROL Acabou de Nascer Mate Agora !!! Sf x: 2225 y: 2080 ");
			SendNotice(temp);


		}

		if ((when.tm_hour == 18 && when.tm_min == 5) && when.tm_sec >= 0 && when.tm_sec <= 2)
		{


			//BOSS_ext3
			GenerateMob(BOSS_ext3, 0, 0);
			sprintf(temp, "BM ROL Acabou de Nascer Mate Agora !!! Kefra x: 3635 y: 3022 ");
			SendNotice(temp);


		}

		if ((when.tm_hour == 21 && when.tm_min == 5) && when.tm_sec >= 0 && when.tm_sec <= 2)
		{


			//BOSS_ext4
			GenerateMob(BOSS_ext4, 0, 0);
			sprintf(temp, "HT ROL Acabou de Nascer Mate Agora !!! Kefra x: 1137 y: 1707 ");
			SendNotice(temp);


		}

		if ((when.tm_hour == 22 && when.tm_min == 5) && when.tm_sec >= 0 && when.tm_sec <= 2)
		{
		...
		if ((when.tm_hour == 1 && when.tm_min == 5) && when.tm_sec >= 0 && when.tm_sec <= 2)
		{
		...
		if ((when.tm_hour == 6 && when.tm_min == 5) && when.tm_sec >= 0 && when.tm_sec <= 2)
		{
		...
		if ((when.tm_hour == 9 && when.tm_min == 5) && when.tm_sec >= 0 && when.tm_sec <= 2)
		{
		...
		if (SecCounter % 1 == 0)
		{
			...
			if (p_BatalhaReal.start == true)
			{
				...
				for (int i = 0; i < 2; i++)
				{
					...
					if (BrTime > 0)
						BrTime--;
					SendClientSignalParmCoord(i, ESCENE_FIELD, _MSG_StartTime, BrTime, 2124, 2140, 1679, 2155);
				}
			}
		}
#pragma endregion

#pragma region  >> GreenTimer Lojinha
		...
		if (SecCounter % 1800 == 0)
		{
			...
			for (int i = 0; i < MAX_USER; i++)
			{
				...
				if (Limitadordelojinha(pUser[i].MacAddress) >= 2)
				{
					SendClientMessage(i, "Limite de [01] auto venda.");
					RemoveTrade(i);
					return;
				}
				if (pMob[i].CheckLojinha > 0)
				...
				if (pUser[i].TradeMode == 1 && pMob[i].CheckLojinha <= 0)
				{
					...
					pMob[i].CheckLojinha = TIMERLOJINHA;//600 = 10 minutos
					SendClientMessage(i, "Voce ganhou +1 ponto de donate digite /donate para ver seu saldo.");
					//sprintf(temp, "Conta : %s Recebeu o item do Evento Lojinha.", pUser[i].AccountName);
					//ItemLog(temp, "-Lojinha-Log", 0);
				}
...
#pragma region Boss 1 Castelo de Gelo
		if ((when.tm_hour == 12 && when.tm_min == 0) && when.tm_sec >= 0 && when.tm_sec <= 2 && (EspelhoLive == 0))
		{
			...
			//Boss 1
			GenerateMob(ESPELHO_RAINHAG, 2133, 2147);

			SendNotice("O BOSS acaba de nascer no PvP de Armia");
		}
#pragma endregion
#pragma region Boss 1 Castelo de Gelo Parte 2
		if ((when.tm_hour == 12 && when.tm_min == 20) && when.tm_sec >= 0 && when.tm_sec <= 2 && (EspelhoLive == 1))
		{
			...
			{
				if (!strcmp(pMob[i].MOB.MobName, "Boss Scorpion"))
					DeleteMob(i, 1);
			}

			SendNotice("Ninguem foi confrontar o Boss e ele fugiu");
		}
#pragma endregion
#pragma region Boss 1 Castelo de Gelo Parte 3
		if ((when.tm_hour == 12 && when.tm_min == 40) && when.tm_sec >= 0 && when.tm_sec <= 2)
		{
			for (int i = MAX_USER; i < MAX_MOB; i++)
			{
			...
			SendNotice("A quest do PvP de Armia foi resetada");
		}
#pragma endregion
#pragma region SORTEIO

		if ((when.tm_min == 4 || when.tm_min == 24 || when.tm_min == 44 || when.tm_min == 54) && Sorteio == 0)
		{
			Sorteio = 1;
		}

		if ((when.tm_min == 0 || when.tm_min == 15 || when.tm_min == 30 || when.tm_min == 50) && Sorteio == 1)
		{
			short num = 0;
			...
			if (num <= 0) // Tem ninguem no servidor
				return;
			...
			int Itens[2][16] =
			{
				//Armas DN
				{
					// Poeira orich - Poeira lac - Poeira orich - Poeira lac
					419, 420, 419, 420,
				},
			};

			STRUCT_ITEM Item;
			Item.sIndex = Itens[0][rand() % 3];
			PutItem(rd, &Item);

			sprintf(temp, "%s recebeu o item %s [Evento Logue&Ganhe do WYD WORLD]", pMob[rd].MOB.MobName, g_pItemList[Item.sIndex].Name);
			SendNotice(temp);
...
#pragma region Evento Monstros Kersef
		if (EventoMobKilled == 0 && EventMobKilled >= 30)
		{
			int _rd = rand() % 11;

			char mob[10][20] = { "LICHBOSS4", "Arqueiro Elfo", "Guerreiro Elfo", "VeridRainhaG", "Lugefer Inf", "BOSS1_END", "BOSS1_END2", "BOSS_CHEFE", "ESCRAVOS", "INFAMUS" };
			int i = 0;

			if (_rd <= 1)
			{
				GenerateMob(6116, 1183, 1720);
			}

			if (_rd == 2)
			{
				i = 1;
				GenerateMob(6117, 0, 0);
			}

			if (_rd == 3)
			{
				i = 2;
				GenerateMob(6118, 0, 0);
			}

			if (_rd == 4)
			{
				i = 3;
				GenerateMob(6119, 0, 0);
			}

			if (_rd == 5)
			{
				i = 4;
				GenerateMob(6120, 0, 0);
			}

			if (_rd == 6)
			{
				i = 5;
				GenerateMob(6121, 0, 0);
			}

			if (_rd == 7)
			{
				i = 6;
				GenerateMob(6122, 0, 0);
			}

			if (_rd == 8)
			{
				i = 7;
				GenerateMob(6123, 0, 0);
			}

			if (_rd == 9)
			{
				i = 8;
				GenerateMob(6124, 0, 0);
			}

			if (_rd >= 10)
			{
				i = 9;
				GenerateMob(6125, 0, 0);
			}
			...
			sprintf(temp, "%s acabou de nascer na Zona Perdida", mob[i]);
...
#pragma region Start Batalha Real
		if (when.tm_wday != 0 && when.tm_wday != 7 && when.tm_min == 59 && when.tm_sec == 59)
		{
			SendNotice("Registro para a Batalha Real iniciada !");
		}
#pragma region Start Batalha Real
		if (when.tm_wday != 0 && when.tm_wday != 7 && when.tm_min == 1 && when.tm_sec == 59)
		{
			SendNotice("Batalha Real! Restam [180] segundos para o fim do Registro ");
		}
#pragma region Start Batalha Real
		if (when.tm_wday != 0 && when.tm_wday != 7 && when.tm_min == 2 && when.tm_sec == 59)
		{
			SendNotice("Batalha Real! Restam [120] segundos para o fim do Registro ");
		}
#pragma region Start Batalha Real
		if (when.tm_wday != 0 && when.tm_wday != 7 && when.tm_min == 3 && when.tm_sec == 59)
		{
			SendNotice("Batalha Real! Restam [60] segundos para o fim do Registro ");
		}
#pragma region Start Batalha Real
		if (when.tm_wday != 0 && when.tm_wday != 7 && when.tm_min == 5)
		{
			bool status = false;
			if (status == false)
			{
				if (BrState == 0 && when.tm_sec >= 0 && when.tm_sec <= 1)
				{
					MSG_Action sm;
					if (p_BatalhaReal.NumMembers >= 2)
					{
						...
						for (int i = 0; i < 20; i++)
						{
							...
							for (i = 0; i < 20; i++) // puxa pra dentro em areas diferentes 
							{
								int rand_ = rand() % 100;
								if (rand_ <= 25)
									DoTeleport(p_BatalhaReal.Members[i].Id, 2127, 2143);
								else if (rand_ <= 50)
									DoTeleport(p_BatalhaReal.Members[i].Id, 2126, 2153);
								else if (rand_ <= 75)
									DoTeleport(p_BatalhaReal.Members[i].Id, 2140, 2153);
								else
									DoTeleport(p_BatalhaReal.Members[i].Id, 2140, 2143);
							}
							p_BatalhaReal.start = true;
							BrState = 1;
							status = true;
							BrTime = 900;
							for (i = 0; i < MAX_ITEM; i++) // fecha o portao de armia
							{
								if (pItem[i].ITEM.sIndex == 4143)
									break;
							}
					...
					if (p_BatalhaReal.NumMembers <= 0) // finaliza a batalha se n houver jogadores suficientes
					{
		...
		if (BrState == 1) // contagem de players dentro da arena
		{
			if (SecCounter % 1 == 0)
			{
				int Users1 = GetUserInArea(2124, 2140, 2147, 2155, temp);
				SendSignalParmArea(2124, 2140, 2147, 2155, ESCENE_FIELD, _MSG_MobLeft, Users1);
			}
		}
#pragma endregion
		if (BrState == 1 && when.tm_wday != 0 && when.tm_wday != 6 && when.tm_min >= 8) // dano em area apartir de certo tempo
		{
			if (BrState == 1 && SecCounter % 6 == 0)
			{
				if (SecCounter % 6 == 0)
				{
...
#pragma region End Batalha Real
		if (BrState == 1 && BrTime == 0 && p_BatalhaReal.start == true) // finaliza a baalha se n houver ganhador
		{
			p_BatalhaReal.start = false;
			memset(&p_BatalhaReal, 0x0, sizeof(p_BatalhaReal));
			ClearArea(2124, 2140, 2146, 2155);
			SendNotice("Batalha Real Finalizada! - Não houve ganhadores");
			BrState = 0;
			BrTime = 0;
			int i = 0;
			for (i = 0; i < MAX_ITEM; i++) //abre o portao de armia
			{
				if (pItem[i].ITEM.sIndex == 4143)
					break;
			}
		...
		{//Pista de Runas entrada
			...
			for(int s = 0; s < 7; s++)
			{
				for(int t = 0; t < 3; t++)
				{
					...
					if(s == 2)
						GenerateMob(RUNEQUEST_LV2_MOB_BOSS, 3392, 1436);
...
/*#pragma region Avisos
		if ((when.tm_min == 40 || when.tm_min == 00) && when.tm_sec == 0)
		{
			SendNotice("Bem vindo(a) ao Wyd Koreano Arch !");
			return;
		}
		if ((when.tm_min == 20 || when.tm_min == 40 || when.tm_min == 00) && when.tm_sec == 5)
		{
			SendNotice("Este servidor so conta com itens arch");
			return;
		}
		if ((when.tm_min == 20 || when.tm_min == 40 || when.tm_min == 00) && when.tm_sec == 10)
		{
			SendNotice("Faca premios nos npcs nas cidades levando medalhas");
			return;
		}
		if ((when.tm_min == 20 || when.tm_min == 40 || when.tm_min == 00) && when.tm_sec == 15)
		{
			SendNotice("Entre nos grupos sem dar diferenca de level digitando /entrar nick e senha");
			return;
		}
		if ((when.tm_min == 20 || when.tm_min == 40 || when.tm_min == 00) && when.tm_sec == 20)
		{
			SendNotice("Abra Uma Lojinha durante 10 Minutos e ganhe gold pode abrir 3 contas");
			return;
		}
		if ((when.tm_min == 20 || when.tm_min == 40 || when.tm_min == 00) && when.tm_sec == 25)
		{
			SendNotice("DUVIDAS , DONATE OU SUPORTE WHATSAP ADM 11 97314-4280");
			return;
		}
...
#pragma region Ataque / fala em   dos mobs
